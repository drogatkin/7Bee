...
public class ClassFileParser extends AbstractParser { 
 ...
 public List getDependencies() {
	List result = new ArrayList(constantPool.length);
    ...
    private FieldOrMethodInfo parseFieldOrMethodInfo()
		...
		FieldOrMethodInfo result = 
			new FieldOrMethodInfo(in.readUnsignedShort(), 
    ...
    private Constant getConstantPoolEntry(int entryIndex)
		...
		if (entryIndex < 0 || entryIndex >= constantPool.length) {
			throw new IOException("Illegal constant pool index : " + 
				entryIndex);
		}
	...
	private void addImport(String importPackage) {
		if ( (importPackage != null) &&
			 (getFilter().accept(importPackage)) ) {
	...
	class FieldOrMethodInfo {
		...
		FieldOrMethodInfo(int accessFlags, 
								 int nameIndex, 
								 int descriptorIndex) {
		...
		public String toString() {
			...
			try {
				...
				String[] types = 
					descriptorToTypes(toUTF8(getDescriptorIndex()));
				for (int t=0; t < types.length; t++) {
					s.append("\n        type = " + types[t]);
				}
